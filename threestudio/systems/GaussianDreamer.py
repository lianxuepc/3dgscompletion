...
class GaussianDreamer(BaseLift3DSystem):
    ...
    class Config(BaseLift3DSystem.Config):
        ...
        partial_path:str = "./data/00.pcd"



    cfg: Config # 这里定义了一个名为 cfg 的实例，其类型为 Config
    ...
    def configure_optimizers(self):
        print("1 configure_optimizers：设置参数，优化起，初始化高斯点云")
    ...
    # 2 ：获取点云坐标，rgb颜色，scale缩放
    def pcb(self):
        print("2 pcb：获取点云坐标，rgb颜色，scale缩放,调用shape-e或者直接从3D模型获取点云")
        # Since this data set has no colmap data, we start with random points
        if self.load_type == 0: # 生成
            coords, rgb, scale = self.shape()
        elif self.load_type == 1: # 模型
            coords, rgb, scale = self.smpl()
        elif self.load_type == 2:
            coords, rgb, scale = self.partial()
        else:
    ...
    def shape(self):
        ...
        print(f'    Coords shape: {coords.shape}, RGB shape: {rgb.shape}')
    ...
    # 3: 从path中读取pcd文件
    def partial(self):
        print(f"3 partial： 从{self.cfg.partial_path}中读取pcd文件，返回coords,rgb,scale")
        point_cloud = o3d.io.read_point_cloud(self.cfg.partial_path)
        # 点数量
        self.num_pts = len(point_cloud.points)
        coords = np.array(point_cloud.points)
        shs = np.random.random((self.num_pts, 3)) / 255.0
        rgb = SH2RGB(shs)
        return coords,rgb,0.5

    # 4：给初始点云增加点
    def add_points(self,coords,rgb):
        print("4 add_points：给初始点云增加点")
    ...
    def on_fit_start(self) -> None:
        print("5 on_fit_start：训练之前设置prompt process和guidance")
        super().on_fit_start()
    ...
    # 7：training_step 里面的 guidance_out['loss_sds'] *self.C(self.cfg.loss['lambda_sds']) 里面的self.C(self.cfg.loss['lambda_sds'])是一个常数
    def forward(self, batch: Dict[str, Any],renderbackground = None) -> Dict[str, Any]:
        print("7 forward：渲染3D Gaussian，返回comp_rgb,depth,opacity等")
    ...
    def on_before_optimizer_step(self, optimizer):
        ...
        with torch.no_grad():
            if self.true_global_step < 12000: # 15000
